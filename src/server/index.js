...
const getStrategy = function (args) {
  const id = args.id;
  return strategyData.filter((strategy) => strategy.id == id)[0];
};

const getStrategies = function () {
  return strategyData;
};

const getRandom = function () {
  const rando = parseInt(Math.floor(Math.random() * 190) + 1, 10);
  return strategyData.filter((strategy) => strategy.id == rando)[0];
};
